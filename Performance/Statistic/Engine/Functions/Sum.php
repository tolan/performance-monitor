...
/**
 * This script defines class for create condition of sum all values for data in statistic select.
 *
 * @author     Martin Kovar
...
class Sum extends AbstractFunction {
    ...
    protected function _addFunction(Select $select, $value, $table, $column, $alias) {
        ...
        $select->columns(
            array($alias => 'IF('.$select->getTableName($table).'.'.$column.' LIKE '.$cleaned.', '.$select->getTableName($table).'.'.$column.', 0)')
        );
    }
}
