...
/**
 * This script defines class for junction between attempt and condition of filter.
 *
 * @author     Martin Kovar
 * @category   Performance
 * @package    Search
 */
class Attempt extends AbstractJunction {
    ...
    protected function file(Select $select, AbstractCondition $condition) {
        ...
        $select->joinInner(array($alias => 'attempt_statistic_data'), 'target.id = '.$alias.'.attemptId', array('file'));
    ...
    protected function line(Select $select, AbstractCondition $condition) {
        $alias = $select->getUniqueTableAlias();
        $select->joinInner(array($alias => 'attempt_statistic_data'), 'target.id = '.$alias.'.attemptId', array('line'));
    ...
    protected function content(Select $select, AbstractCondition $condition) {
        $alias = $select->getUniqueTableAlias();
        $select->joinInner(array($alias => 'attempt_statistic_data'), 'target.id = '.$alias.'.attemptId', array('content'));
    ...
    protected function immersion(Select $select, AbstractCondition $condition) {
        $alias = $select->getUniqueTableAlias();
        $select->joinInner(array($alias => 'attempt_data'), 'target.id = '.$alias.'.attemptId', array('immersion'));
